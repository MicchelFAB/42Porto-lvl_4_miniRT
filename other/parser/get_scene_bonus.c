...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/15 16:19:54 by bmonteir          #+#    #+#             */
/*   Updated: 2024/05/19 19:32:33 by mamaral-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_get_patern(char **split, t_scene *scene)
{
	...
	scene->new->material.pattern.a = ft_color(
			ft_normalize_color(scene->new->material.pattern.a.x),
			ft_normalize_color(scene->new->material.pattern.a.y),
			ft_normalize_color(scene->new->material.pattern.a.z));
	scene->new->material.pattern.b = ft_get_colour(split[3], RGB_BONUS, scene);
	scene->new->material.pattern.b = ft_color(
			ft_normalize_color(scene->new->material.pattern.b.x),
			ft_normalize_color(scene->new->material.pattern.b.y),
			ft_normalize_color(scene->new->material.pattern.b.z));
	scene->new->material.pattern = ft_create_pattern(
			scene->new->material.pattern.type,
			scene->new->material.pattern.a, scene->new->material.pattern.b);
	scene->bonus_qtys[0]++;
}
...
void	ft_get_reflective(char **split, t_scene *scene)
{
	...
	if (scene->new->material.shininess < 0
		|| scene->new->material.shininess > 2000)
		ft_msg_error(scene, "Invalid value for shininess");
	scene->new->material.reflective = ft_get_size(split[2], RF_REF, scene);
	if (scene->new->material.reflective < 0
		|| scene->new->material.reflective > 1)
...
void	ft_get_glass(char **split, t_scene *scene)
{
	...
	if (scene->new->material.transparency < 0
		|| scene->new->material.transparency > 1)
		ft_msg_error(scene, "Invalid value for transparency");
	scene->new->material.refractive_index = ft_get_size(split[2],
			GL_REFLEC, scene);
