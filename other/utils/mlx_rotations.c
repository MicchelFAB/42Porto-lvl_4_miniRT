...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/13 13:05:35 by mamaral-          #+#    #+#             */
/*   Updated: 2024/05/19 15:04:21 by mamaral-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_matrix	ft_rotate_camera(t_world *cam)
{
	...
	if (cam->key.row4.a)
		tra = ft_multiply_matrix(tra, ft_create_rotation_matrix('y',
					-M_PI_4 / 4));
	else if (cam->key.row4.d)
		tra = ft_multiply_matrix(tra, ft_create_rotation_matrix('y',
					M_PI_4 / 4));
	if (cam->key.row3.w)
		tra = ft_multiply_matrix(tra, ft_create_rotation_matrix('x',
					-M_PI_4 / 4));
	else if (cam->key.row4.s)
		tra = ft_multiply_matrix(tra, ft_create_rotation_matrix('x',
					M_PI_4 / 4));
	if (cam->key.row3.q)
		tra = ft_multiply_matrix(tra, ft_create_rotation_matrix('z',
					-M_PI_4 / 4));
